...
//                editmode: 'click',
                ...
                columns: [
                  ...
                  { text: 'numComitente', datafield: 'numComitente', width: 70},
                  { text: 'comitente', datafield: 'comitente', width: 180},
                  { text: 'fechaLiquidacion', datafield: 'fechaLiquidacion', width: 148},
                  { text: 'tipoOperacionBurs', datafield: 'tipoOperacionBurs', width: 148, hidden:true},
                  { text: 'tipoOperacionBursDesc', datafield: 'tipoOperacionBursDesc', width: 148},

                  { text: 'codMoneda', datafield: 'codMoneda', width: 80, hidden:true},
                  { text: 'monedaDescripcion', datafield: 'monedaDescripcion', width: 80},
                  
                  { text: 'cantidad', datafield: 'cantidad', width: 100},

                  
                  { text: 'codEspecie', datafield: 'codEspecie', width: 100, hidden:true},
                  { text: 'especieAbreviatura', datafield: 'especieAbreviatura', width: 100, hidden:true},
...
//                  { text: 'CI', datafield: 'CI', width: 80, cellclassname: cellclassname},
                  ...
                  { text: 'fechaActualizacion', datafield: 'fechaActualizacion', width: 90, cellsformat: 'dd/MM/yyyy'},
                  { text: 'procesadores', datafield: 'procesadores', width: 110},
...
//        $("#enviarMercadoButton").jqxButton({ width: '160', theme: theme, disabled: true });
        ...
        $("#enviarMercadoButton").click(function(){
            ...
            $("#grilla").ajaxloader();
            $.post('/minuta/previewMercado', datos, function(data){
                ...
                new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                    buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) {
                        if (val == 's'){
                            $.post('/minuta/enviarMercado', datos, function(data){
                                var titleClass;
                                if (data.exito == 0){
                                    titleClass = 'error';
                                } else {
        ...
        */
            ...
            new Messi('Desea procesar las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) {
                    if (val == 's'){
                        ...
                        $.post('/minuta/procesarOrdenes', datos, function(data){
                            new Messi(data.resultado, {title: 'Mensaje', modal: true,
            ...
            new Messi('Desea anular las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) {
                    if (val == 's'){
                        ...
                        $.post('/minuta/anularOrdenes', datos, function(data){
                            new Messi(data.resultado, {title: 'Mensaje', modal: true,
                                buttons: [{id: 0, label: 'Cerrar', val: 'X'}], titleClass: 'error'});
                            $('#grilla').jqxGrid('updatebounddata');
                            $('#grilla').jqxGrid('clearselection');
        ...
        $("#editarButton").click(function(){
            $.redirect('/minuta/editar', {'id': id, origen: 'procesar'});
        });
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; margin-left: 0px; margin-bottom: 10px; text-align: left; vertical-align: text-bottom;'><ul>Procesar Minutas</ul></div>
<br>
<div id="grilla"></div>
<div id="botonera">
