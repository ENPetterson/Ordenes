...
<!--                <tr>
                    ...
                    <td><input type="text" id="instrumentoCanjelocal"></td>
                </tr>-->
...
//        $("#instrumentoCanjelocal").jqxInput({height: '20px', width: '190px', theme: theme});
...
//                    { text: 'Titulo No Competitivo Persona Fisica', datafield: 'tituloNCF', width: 0, hidden: true}
        ...
        if ($("#id").val() == 0){
            ...
            datos = {
                cierrecanjelocal_id: $("#id").val()
            };
            $.post('/canjelocal/getCierre', datos, function(data){
...
//                $("#instrumentoCanjelocal").val(data.instrumento);
            }, 'json');
        };
...
//            { input: '#instrumentoCanjelocal', message: 'Debe ingresar el nombre del instrumento!',  rule: 'required' }
            /*
            { input: '#instrumentoCanjelocal', message: 'No existe un instrumento con ese nombre!',  rule: function(){
                var resultado;
                jQuery.ajaxSetup({async:false});
                $.post('/esco/existeInstrumento', {instrumento: $('#instrumentoCanjelocal').val()}, function(data){
...
//                    instrumento: $("#instrumentoCanjelocal").val()
                };
                $.post('/canjelocal/saveCierre', datos, function(data){
                    if (data.id > 0){
                        $.redirect('/canjelocal/cierre');
                    } else {
