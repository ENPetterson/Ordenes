<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#cierre').on('change', function (event)        {  
            source.data = {cierrecanje_id: event.args.item.value};
...
//                { name: 'cantidadAcrecerSegunda', type: 'number'},
        ...
        {		
                ...
                height: 400,
                columns: [
                        { text: 'Id', datafield: 'id', width: 50, cellsalign: 'right', cellsformat: 'd', aggregates: ['count'] },
                        { text: 'Nro Comitente', datafield: 'numComitente', width: 70},
//                        { text: 'Comis', datafield: 'comision', width: 60, cellsalign: 'right', cellsformat: 'd4'},
                        { text: 'Cantidad', datafield: 'cantidad', width: 100, cellsalign: 'right', cellsformat: 'd', aggregates: ['sum'] },
                        { text: 'Arancel', datafield: 'arancel', width: 60, cellsalign: 'right', cellsformat: 'd4'},
...
//                        { text: 'Cant A Crec 2da', datafield: 'cantidadAcrecerSegunda', width: 140, cellsalign: 'right', cellsformat: 'd', aggregates: ['sum'] },
        ...
        $("#enviarMercadoButton").click(function(){
            ...
            $("#grilla").ajaxloader();
            $.post('/canje/previewMercado', datos, function(data){
                ...
                new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                    buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                        if (val == 's'){
                            $.post('/canje/enviarMercado', datos, function(data){
                                var titleClass;
                                if (data.exito == 0){
                                    titleClass = 'error';
                                } else {
            ...
            new Messi('Desea anular las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/canje/anularOrdenes', datos, function(data){
                            new Messi(data.resultado, {title: 'Mensaje', modal: true,
        ...
        $("#editarButton").click(function(){
            $.redirect('/canje/editar', {'id': id, origen: 'procesar'});
        });
        ...
        $("#aceptarSantander").click(function(){
            var datos = {ordenes: enviar};
            $.post('/canje/enviarSantander', datos, function(data){
                var titleClass;
                if (data.exito == 0){
                    titleClass = 'error';
                } else {
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Procesar Canjees</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
