<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#cierre').on('change', function (event)        {  
            source.data = {cierrelicitacion_id: event.args.item.value};
        ...
        var source = {
            ...
            cache: false,
            url: '/licitacion/procesarGrilla',
            data: {cierrelicitacion_id: cierre_id},
            type: 'post'
        };
        ...
        $("#enviarMercadoButton").click(function(){
            ...
            $("#grilla").ajaxloader();
            $.post('/licitacion/previewMercado', datos, function(data){
                ...
                new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                    buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                        if (val == 's'){
                            $.post('/licitacion/enviarMercado', datos, function(data){
                                var titleClass;
                                if (data.exito == 0){
                                    titleClass = 'error';
                                } else {
            ...
            new Messi('Desea anular las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/licitacion/anularOrdenes', datos, function(data){
                            new Messi(data.resultado, {title: 'Mensaje', modal: true,
        ...
        $("#editarButton").click(function(){
            $.redirect('/licitacion/editar', {'id': id, origen: 'procesar'});
        });
        ...
        var srcGrillaPreviewSantander = {
            ...
            url: '/licitacion/previewSantander',
            data: {ordenes: enviar},
            type: 'post'
        };
        ...
        $("#aceptarSantander").click(function(){
            var datos = {ordenes: enviar};
            $.post('/licitacion/enviarSantander', datos, function(data){
                var titleClass;
                if (data.exito == 0){
                    titleClass = 'error';
                } else {
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Procesar Licitación</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
