...
//                        { text: 'Tipo Per', datafield: 'tipoPersona', width: 100},
        ...
        $("#nuevoButton").click(function(){
            $.redirect('/licitacion/editar', {'id': 0, origen: 'licitacion'});
        });
        
        $("#editarButton").click(function(){
            $.redirect('/licitacion/editar', {'id': id, origen: 'licitacion'});
        });
            ...
            new Messi('Desea borrar las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/licitacion/delOrden', datos, function(data){
            ...
            new Messi('Desea enviar las ordenes seleccionadas ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/licitacion/enviarOrdenes', datos, function(data){
                            var titleClass;
                            if (data.exito == 0){
                                titleClass = 'error';
                            } else {
...
////////////////////////////////////////////////////////////////////////////////   
        ...
        $('#archivoExcel').uploadifive({
            ...
            'onUploadComplete': function(file) {
                $('#grilla').ajaxloader();
                $.post('/licitacion/grabarExcel', { file: file.name, cierre: $("#cierre").jqxDropDownList('getSelectedItem').value}, function(msg){
                    var titleClass;
                    var mensaje;
                    var title;
                    if(msg.resultado == 'OK'){
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Grilla Licitación</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
