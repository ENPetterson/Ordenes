...
//                { name: 'cierretreasuries_id'}
            ],
            cache: false,
            url: '/treasuries/grilla',
...
//                        { text: 'cierretreasuries_id', datafield: 'cierretreasuries_id', width: 100}
        ...
        $("#nuevoButton").click(function(){
            $.redirect('/treasuries/editar', {'id': 0, origen: 'treasuries'});
        });
        
        $("#editarButton").click(function(){
            $.redirect('/treasuries/editar', {'id': id, origen: 'treasuries'});
        });
            ...
            new Messi('Desea borrar las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/treasuries/delOrden', datos, function(data){
            ...
            new Messi('Desea enviar las ordenes seleccionadas ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/treasuries/enviarOrdenes', datos, function(data){
                            var titleClass;
                            if (data.exito == 0){
                                titleClass = 'error';
                            } else {
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Grilla Treasuries</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
