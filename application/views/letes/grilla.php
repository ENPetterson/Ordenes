<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#cierre').on('change', function (event)        {  
            source.data = {cierreletes_id: event.args.item.value};
        ...
        $("#nuevoButton").click(function(){
            $.redirect('/letes/editar', {'id': 0, origen: 'letes'});
        });
        
        $("#editarButton").click(function(){
            $.redirect('/letes/editar', {'id': id, origen: 'letes'});
        });
            ...
            new Messi('Desea borrar las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/letes/delOrden', datos, function(data){
            ...
            new Messi('Desea enviar las ordenes seleccionadas ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/letes/enviarOrdenes', datos, function(data){
                            var titleClass;
                            if (data.exito == 0){
                                titleClass = 'error';
                            } else {
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Grilla letes</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
