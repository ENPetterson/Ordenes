...
//        var cierre_id = 0;
        var minimos = [];
        var minimo = 0;
        var cierregenerartxt_id = 0;
        
        $("#filaPrecio").hide();
        
        $("#ventanaGenerartxt").jqxWindow({showCollapseButton: false, height: 500, width: 470, theme: theme, resizable: false, keyboardCloseKey: -1});
...
//        $("#tipoInversor").jqxDropDownList({ width: '300px', height: '25px', source: tipoInversor, theme: theme, placeHolder: 'elija el tipo de inversor'});
        ...
        var srcPlazos = {
            ...
            data: {cierregenerartxt_id: cierregenerartxt_id},
            async: false,
            type: 'POST',
            id: 'id',
            url: '/generartxt/getPlazosEspecies'
        }
...
//        $("#plazo").jqxDropDownList({ width: '110px', height: '25px', source: DAMonedas, theme: theme, placeHolder: 'elija plazo'});
        ...
        $("#precio").jqxNumberInput({ width: '110px', height: '25px', decimalDigits: 6, digits: 6, groupSeparator: ' ', max: 999999.999999, theme: theme});
...
//               elPlazo = 
        ...
        */
        ...
        if ($("#id").val() == 0){
            $("#titulo").text('Nueva Orden Generar Txt');
            $("#filaCable").hide();
            
             
            var url = '/generartxt/getCierreActual';
            $.post(url, {}, function(cierre){
                var comboPlazos;
            
                console.log(cierre.plazos);

                
                comboPlazos = cierre.plazos;
                
                
//                minimos = cierre.minimos.split(",");
                $("#plazo").jqxDropDownList('clear'); 
                $.each(comboPlazos, function(index,value){
                    $("#plazo").jqxDropDownList('addItem', value.plazo ); 
                });
            }, 'json');
            ...
            $.post('/generartxt/getOrden', datos, function(data){
                cierregenerartxt_id = data.cierregenerartxt_id;
...
//                $("#tipoInversor").val(data.tipoInversor);
                ...
                var datos = {cierregenerartxt_id: cierregenerartxt_id};
                url = '/generartxt/getCierre';
