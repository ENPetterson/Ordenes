<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#cierre").on('bindingComplete', function(event){
            $.post('/generartxt/getCierreActual', function(cierre){
                if (!cierre.cerrado){
        ...
        $('#cierre').on('change', function (event)        {  
            source.data = {cierregenerartxt_id: event.args.item.value};
        ...
        var source = {
            ...
            cache: false,
            url: '/generartxt/procesarGrilla',
            data: {cierregenerartxt_id: cierre_id},
            type: 'post'
        };
        ...
        {		
                ...
                columns: [
                        ...
                        { text: 'Plazo', datafield: 'plazoplazo', width: 70},
                        { text: 'Especie', datafield: 'plazoespecie', width: 70},
        ...
        $("#enviarMercadoButton").click(function(){
            ...
            $("#grilla").ajaxloader();
            $.post('/generartxt/previewMercado', datos, function(data){
                ...
                new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                    buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                        if (val == 's'){
                            $.post('/generartxt/enviarMercado', datos, function(data){
                                var titleClass;
                                if (data.exito == 0){
                                    titleClass = 'error';
                                } else {
            ...
            new Messi('Desea anular las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/generartxt/anularOrdenes', datos, function(data){
                            new Messi(data.resultado, {title: 'Mensaje', modal: true,
        ...
        $("#editarButton").click(function(){
            $.redirect('/generartxt/editar', {'id': id, origen: 'procesar'});
        });
        ...
        var srcGrillaPreviewSantander = {
            ...
            url: '/generartxt/previewSantander',
            data: {ordenes: enviar},
            type: 'post'
        };
        ...
        $("#aceptarSantander").click(function(){
            var datos = {ordenes: enviar};
            $.post('/generartxt/enviarSantander', datos, function(data){
                var titleClass;
                if (data.exito == 0){
                    titleClass = 'error';
                } else {
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Procesar Generacion Txt</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
