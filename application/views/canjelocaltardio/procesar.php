...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var srcCierre = {
            ...
            id: 'id',
            url: '/canjelocaltardio/getCierres',
            async: false
        };
        ...
        $('#cierre').on('change', function (event)        {  
            source.data = {cierrecanjelocaltardio_id: event.args.item.value};
...
//                { name: 'envio'},
            ...
            cache: false,
            url: '/canjelocaltardio/procesarGrilla',
...
//                        { text: 'Cant A Crec 2da', datafield: 'cantidadAcrecerSegunda', width: 140, cellsalign: 'right', cellsformat: 'd', aggregates: ['sum'] },
        ...
        $("#grilla").on("bindingcomplete", function (event){
            ...
            $("#grilla").jqxGrid('localizestrings', localtardioizationobj);
        }); 
        ...
        $("#enviarMercadoButton").click(function(){
            ...
            $("#grilla").ajaxloader();
            $.post('/canjelocaltardio/previewMercado', datos, function(data){
                ...
                new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                    buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                        if (val == 's'){
                            $.post('/canjelocaltardio/enviarMercado', datos, function(data){
                                var titleClass;
                                if (data.exito == 0){
                                    titleClass = 'error';
                                } else {
        ...
        $("#enviarArchivoButton").click(function(){
            ...
            var datos = {ordenes: enviar};
            $("#grilla").ajaxloader();
            $.post('/canjelocaltardio/previewArchivo', datos, function(data){                
                var resultado = data;
                if(resultado){
                    $.redirect('/canjelocaltardio/getDescargarAchivo', {archivo: resultado});
//                    $.redirect('/flujo/getDescargarLog', {'logName': logName});                    
                ...
                new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                    buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                        if (val == 's'){
                            $.post('/canjelocaltardio/enviarArchivo', datos, function(data){
                                var titleClass;
                                if (data.exito == 0){
                                    titleClass = 'error';
                                } else {
...
////////////////////////////////////////////////////////////////////////////////   
        ...
        $('#procesarExcel').uploadifive({
            ...
            'onUploadComplete': function(file) {
                $('#grilla').ajaxloader();
                $.post('/canjelocaltardio/procesarExcel', { file: file.name, cierre: $("#cierre").jqxDropDownList('getSelectedItem').value}, function(msg){
                    var titleClass;
                    var mensaje;
                    var title;
                    if(msg.resultado == 'OK'){
                            ...
                            new Messi('Ha enviado los datos al mercado ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                                    if (val == 's'){
                                        ...
                                        console.log('Aca están los datos del resultado y se vá a descargar');
                                        console.log(datos);
                                        console.log(msg);
                                        
//                                        console.log('lalala');
                                        $.post('/canjelocaltardio/procesarExcelAchivoDescargado', datos, function(data){
                                            var titleClass;
                                            if (data.exito == 0){
                                                titleClass = 'error';
                                            } else {
...
////////////////////////////////////////////////////////////////////////////////                   
            ...
            new Messi('Desea anular las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/canjelocaltardio/anularOrdenes', datos, function(data){
                            new Messi(data.resultado, {title: 'Mensaje', modal: true,
        ...
        $("#editarButton").click(function(){
            $.redirect('/canjelocaltardio/editar', {'id': id, origen: 'procesar'});
        });
        ...
        var srcGrillaResumen = {
            ...
            url: '/canjelocaltardio/grillaResumen',
            data: {ordenes: enviar},
            type: 'post'
        };
        ...
        var srcGrillaPreviewSantander = {
            ...
            url: '/canjelocaltardio/previewSantander',
            data: {ordenes: enviar},
            type: 'post'
        };
        ...
        $("#aceptarSantander").click(function(){
            var datos = {ordenes: enviar};
            $.post('/canjelocaltardio/enviarSantander', datos, function(data){
                var titleClass;
                if (data.exito == 0){
                    titleClass = 'error';
                } else {
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Procesar Canjelocaltardioes</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
