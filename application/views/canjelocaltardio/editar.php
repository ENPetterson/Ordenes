...
<div id="ventanaCanjelocaltardio">
    <div id="titulo">
        Editar Orden Canjelocaltardio
    </div>
    <div>
        <form id="form">
            <table>
...
<!--                <tr class="cantidadAcrecerSegunda">
                ...
                <tr>
                    <td style="padding-right: 10px; padding-bottom: 10px">EUROCLEAR/DTC:</td>
...
<script>
    $(function(){
        ...
        var formOK = false;
        var comitente = false;
        var cierrecanjelocaltardio_id = 0;
        var plazoCargado = 0;
        var posicion = false;
            var especie = '';
        var item = '';
        ...
        $("#ventanaCanjelocaltardio").jqxWindow({showCollapseButton: false, height: 500, width: 500, theme: theme, resizable: false, keyboardCloseKey: -1});
...
//                }, 'json');
        ...
        var srcPlazos = {
            ...
            data: {cierrecanjelocaltardio_id: cierrecanjelocaltardio_id},
            async: false,
            type: 'POST',
            id: 'id',
            url: '/canjelocaltardio/getPlazosEspecies'
        }
...
//        $("#plazo").jqxDropDownList({ width: '110px', height: '25px', source: DAMonedas, theme: theme, placeHolder: 'elija plazo'});
        ...
        var srcBonos = {
            ...
            data: {
                    plazocanjelocaltardio_id: $("#plazo").val()
                },
            async: false,
            type: 'POST',
            id: 'id',
            url: '/canjelocaltardio/getPlazosBono'
        }
...
//        });
        ...
        function calcularPosicion(event){
            ...
            $.post('/canjelocaltardio/getEspecie', { plazo: $("#plazo").val() }, function(data){
                especie = data;
                $.post('/esco/getPosicion', {numComitente: value, especie: especie}, function(pPosicion){
                    ...
                    posicion = pPosicion;
                    if (pPosicion){
                        $("#cantidad").val(pPosicion.Cantidad);
                        $("#posicion").val(pPosicion.Cantidad);

                        $('#numComitente').jqxNumberInput('focus');
...
//        }); 
        ...
        if ($("#id").val() == 0){
            $("#titulo").text('Nueva Orden Canjelocaltardioes');
            $("#filaCable").hide();
        } else {
            $("#titulo").text('Editar Orden Canjelocaltardioes');
            datos = {
                id: $("#id").val()
            };
            $.post('/canjelocaltardio/getOrden', datos, function(data){
                cierrecanjelocaltardio_id = data.cierrecanjelocaltardio_id;
...
//                }
        ...
        $('#form').jqxValidator({ rules: [
                ...
                { input: '#cantidad', message: 'Debe ingresar cantidad ayor a 0!', action: 'keyup, blur',  rule: function(){
                    if ( $("#cantidad").val() == 0 )  {
                        return false;
                    } else {
                        return true;
...
//                }},
        ...
        $('#aceptarButton').bind('click', function () {
            ...
            if (formOK){                
                $('#ventanaCanjelocaltardio').ajaxloader();
//                var segundaParte = 0;
...
//                    cantidadAcrecerSegunda: cantidadAcrecerSegunda,
                ...
                $.post('/canjelocaltardio/saveOrden', datos, function(data){
                    if (data.id > 0){
                        ...
                        $('#ventanaCanjelocaltardio').ajaxloader('hide');
                    }
                }, 'json');
            }
        });                
