...
<!--                <tr>
                    ...
                    <td><input type="text" id="instrumentoCanjelocaltardio"></td>
                </tr>-->
                
                <tr>
                    <td style="padding-right: 10px; padding-bottom: 10px">Pausar Cierre:</td>
                    <td style="padding-right: 10px; padding-bottom: 10px; padding-top: 10px"><div id="pausarCierre"></div></td>
                </tr>                
                <tr>
...
<script>
    $(function(){
        ...
        $("#ventanaCierre").jqxWindow({showCollapseButton: false, height: '500px', width: '600px', theme: theme,
...
//        $("#instrumentoCanjelocaltardio").jqxInput({height: '20px', width: '190px', theme: theme});
        ...
        $("#grillaPlazos").jqxGrid( {		
                ...
                columns: [
                    ...
                    { text: 'Moneda', datafield: 'moneda', width: 50},
                    { text: 'Plazo', datafield: 'plazo', width: 120 },
                    { text: 'Especie', datafield: 'especie', width: 200},
...
//                    { text: 'Titulo No Competitivo Persona Fisica', datafield: 'tituloNCF', width: 0, hidden: true}
        ...
        $("#grillaPlazos").on("bindingcomplete", function (event){
            var localtardioizationobj = getLocalization();
            $("#grillaPlazos").jqxGrid('localzestrings', localtardioizationobj);
        });  
        ...
        if ($("#id").val() == 0){
            ...
            datos = {
                cierrecanjelocaltardio_id: $("#id").val()
            };
            $.post('/canjelocaltardio/getCierre', datos, function(data){
...
//                $("#instrumentoCanjelocaltardio").val(data.instrumento);
            }, 'json');
        };
...
//            { input: '#instrumentoCanjelocaltardio', message: 'Debe ingresar el nombre del instrumento!',  rule: 'required' }
            /*
            { input: '#instrumentoCanjelocaltardio', message: 'No existe un instrumento con ese nombre!',  rule: function(){
                var resultado;
                jQuery.ajaxSetup({async:false});
                $.post('/esco/existeInstrumento', {instrumento: $('#instrumentoCanjelocaltardio').val()}, function(data){
...
//                    instrumento: $("#instrumentoCanjelocaltardio").val()
                };
                $.post('/canjelocaltardio/saveCierre', datos, function(data){
                    if (data.id > 0){
                        $.redirect('/canjelocaltardio/cierre');
                    } else {
...
//            { value: 'u$s', label: 'Dolar'}
        ...
        $("#moneda").jqxDropDownList({ width: '300px', height: '25px', source: monedas, theme: theme, placeHolder: 'elija la moneda', selectedIndex: 0, disabled: true});

        $("#plazo").jqxNumberInput({ width: '300px', height: '20px', theme:theme, digits:6, groupSeparator: '', decimalDigits: 0});
