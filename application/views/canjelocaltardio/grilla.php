...
//                { name: 'cantidadAcrecerSegunda', type: 'number'},
            ...
            cache: false,
            url: '/canjelocaltardio/grilla',
            data: {cierrecanjelocaltardio_id: cierre_id},
            type: 'post'
        };
...
//                        { text: 'Cant a Crec 2da', datafield: 'cantidadAcrecerSegunda', width: 140, cellsalign: 'right', cellsformat: 'd', aggregates: ['sum']},
        ...
        $("#nuevoButton").click(function(){
            $.redirect('/canjelocaltardio/editar', {'id': 0, origen: 'canjelocaltardio'});
        });
        
        $("#editarButton").click(function(){
            $.redirect('/canjelocaltardio/editar', {'id': id, origen: 'canjelocaltardio'});
        });
            ...
            new Messi('Desea borrar las ordenes ' + enviar.join(', ') + ' ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                    if (val == 's'){
                        ...
                        $.post('/canjelocaltardio/delOrden', datos, function(data){
...
//            buttons: [{id: 0, label: 'Cerrar', val: 'X'}], titleClass: titleClass});
            ...
            $.post('/canjelocaltardio/comprobarEstadoCierre', datos, function(data){
                if(data == 'true'){
                    var titleClass;
                    titleClass = 'success';
                    new Messi('Estamos realizando procesos en el sistema. Aguarde unos minutos e intente nuevamente', {title: 'Mensaje', modal: true,
                    ...
                    new Messi('Desea enviar las ordenes seleccionadas ?' , {title: 'Confirmar',titleClass: 'warning', modal: true,
                        buttons: [{id: 0, label: 'Si', val: 's'}, {id: 1, label: 'No', val: 'n'}], callback: function(val) { 
                            if (val == 's'){
                                ...
                                $.post('/canjelocaltardio/enviarOrdenes', datos, function(data){
                                    var titleClass;
                                    if (data.exito == 0){
                                        titleClass = 'error';
                                    } else {
...
////////////////////////////////////////////////////////////////////////////////   
        ...
        $('#archivoExcel').uploadifive({
            ...
            'onUploadComplete': function(file) {
                $('#grilla').ajaxloader();
                $.post('/canjelocaltardio/grabarExcel', { file: file.name, cierre: $("#cierre").jqxDropDownList('getSelectedItem').value}, function(msg){
                    var titleClass;
                    var mensaje;
                    var title;
                    if(msg.resultado == 'OK'){
...
<div id="cierre"></div>
<br>
<div id="sistema" style='float: left; vertical-align: text-bottom; text-align: left;'><ul>Grilla Canjelocaltardioes</ul></div>
<br>
<br>
<div id="grilla"></div>
<div id="botonera">
