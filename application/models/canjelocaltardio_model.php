...
class Canjelocaltardio_model extends CI_Model{
...
//    public $cantidadAcrecerSegunda;
    ...
    public $cierrecanjelocaltardio_id;
    public $fechahora;
    ...
    public function saveOrden(){
        ...
        $orden = R::load('canjelocaltardio', $this->id);
        if ($orden->id == 0 ){
            ...
            if (isset($cierreActual['cerrado'])){
                ...
                $cierre = R::load('cierrecanjelocaltardio', $cierreActual['id']);
                $orden->cierrecanjelocaltardio = $cierre;
            }
...
//        $orden->cantidadAcrecerSegunda = $this->cantidadAcrecerSegunda;
    ...
    public function delOrden(){
        foreach ($this->ordenes as $id){
            $orden = R::load('canjelocaltardio', $id);
            R::trash($orden);
        }
    }
    ...
    public function getPlazos(){
        if ($this->cierrecanjelocaltardio_id > 0){
            $sql = "select id, plazo, especie from plazocanjelocaltardio where cierrecanjelocaltardio_id = ? order by plazo";
            $plazos = R::getAll($sql, array($this->cierrecanjelocaltardio_id));
        } else {
            $sql = "select id, plazo, especie from plazocanjelocaltardio where cierrecanjelocaltardio_id = (SELECT id FROM cierrecanjelocaltardio where fechahora > NOW() order by fechahora limit 0,1) order by plazo";
            $plazos = R::getAll($sql);
        }
        return $plazos;
    }
    ...
    public function getEspecie(){
        ...
        $sql = 'select * from plazocanjelocaltardio where id = ? order by plazo';
    ...
    public function getPlazosEspecies(){
        if ($this->cierrecanjelocaltardio_id > 0){
            $sql = 'select id, plazo, CONCAT(plazo, " ", especie) as especie from plazocanjelocaltardio where cierrecanjelocaltardio_id = ? order by plazo';
            $plazos = R::getAll($sql, array($this->cierrecanjelocaltardio_id));
        } else {
            $sql = 'select id, plazo, CONCAT(plazo, " ", especie) as especie from plazocanjelocaltardio where cierrecanjelocaltardio_id = (SELECT id FROM cierrecanjelocaltardio where fechahora > NOW() order by fechahora limit 0,1) order by plazo';
            $plazos = R::getAll($sql);
        }
        return $plazos;
    }
    ...
    public function getPlazosBono(){
        if ($this->plazocanjelocaltardio_id > 0){
            $sql = 'select id, plazocanjelocaltardio_id, plazodescripcion, descripcion, tipo, cierrecanjelocaltardio_id from plazocanjelocaltardiobono where plazocanjelocaltardio_id = ? order by id';
            $plazos = R::getAll($sql, array($this->plazocanjelocaltardio_id));
        } 
        return $plazos;
    }
    ...
    public function grilla(){
        $sql = "select p.id,
                ...
                p.fhenvio,
                p.estaConfirmado
         from   canjelocaltardio p
         join   estadoorden eo 
         on     p.estado_id = eo.id
         join   cierrecanjelocaltardio c
         on     p.cierrecanjelocaltardio_id = c.id
         LEFT join   usuario u
         on     p.usuario_id = u.id
         
         LEFT JOIN plazocanjelocaltardio pla
         on     pla.id = p.plazo

         where  p.usuario_id = ?
         and    p.cierrecanjelocaltardio_id = ?
         order by p.fhmodificacion desc"; 
        
        $resultado = R::getAll($sql, array($this->usuario_id, $this->cierrecanjelocaltardio_id));
    ...
    public function enviarOrdenes(){        
        ...
        foreach ($this->ordenes as $id) {
            $orden = R::load('canjelocaltardio', $id);
            $fechaCierre = new DateTime($orden->cierrecanjelocaltardio->fechahora);
...
//                            <p>Orden Canje Local Tardío Anulada </p>
//                            <p>Estimado: </p>
//                            <p>Se ha anulado una Orden de Canje Local Tardío. {$id} {$usuario['nombre']} {$usuario['apellido']}, Comitente: {$orden['numComitente']}, Especie: {$orden['especie']} </p>
//                            <p>Saludos.</p>";
//
...
//            $respuesta = $this->Mailing_model->enviarMail($textoEmail, 'Allaria Ledesma', 'no-responder@allaria.com.ar', 'Ordenes Canje Local Tardío', $mailTo, 'Ordenes Canje Local Tardío', array(), array());
...
//            $mailTo = array('micaela.petterson@allaria.com.ar');
    ...
    public function getCierre(){
        ...
        $cierreBean = R::load('cierrecanjelocaltardio', $this->cierrecanjelocaltardio_id);
        $cierre = $cierreBean->export();
        $sql = "select * from plazocanjelocaltardio where cierrecanjelocaltardio_id = ? order by moneda, plazo";
    ...
    public function saveCierre(){
        $cierre = R::load('cierrecanjelocaltardio', $this->cierrecanjelocaltardio_id);
...
//        $cierre->instrumento = $this->instrumento;
        ...
        foreach ((array) $this->plazos as $plazoItem) {
            $plazo = R::load('plazocanjelocaltardio', $plazoItem['id']);
...
//            $plazo->tituloNCF = $plazoItem['tituloNCF'];
            $plazo->cierrecanjelocaltardio = $cierre;
            R::store($plazo);
        }
        
        foreach ((array) $this->plazosBorrar as $plazocanjelocaltardio_id){
            $plazo = R::load('plazo', $plazocanjelocaltardio_id);
            R::trash($plazo);
        }
    ...
    public function comprobarEstadoCierre(){
        ...
        $sql = "SELECT pausarCierre FROM cierrecanjelocaltardio where id = ?";
    ...
    public function delCierre(){
        $cierre = R::load('cierrecanjelocaltardio', $this->cierrecanjelocaltardio_id);
        $cierre->ownPlazocanjelocaltardio = array();
        R::store($cierre);
        R::trash($cierre);
    }
    
    public function getCierreActual(){
        $cierreActual = R::findOne('cierrecanjelocaltardio', 'fechahora > NOW() order by fechahora' );
        if (is_null($cierreActual)){
            return array('cerrado'=>true);
        } else {
            $this->cierrecanjelocaltardio_id = $cierreActual->id;
    ...
    public function getCierres(){
        $cierres = R::getAll('select * from cierrecanjelocaltardio order by fechahora desc');
        return $cierres;
    }
    ...
    public function procesarGrilla(){
        ...
        $resultado = R::getAll($sql, array($this->cierrecanjelocaltardio_id));
...
//        $ordenes_in = implode(',', $this->ordenes);
//        $sql = "SELECT  cierrecanjelocaltardio_id,
//                        tipopersona,
//                        numcomitente,
//                        cantidad,
//                        cuit,
//                        FROM    canjelocaltardio
//                WHERE   id in ({$ordenes_in})
//                ORDER BY cierrecanjelocaltardio_id ";
//        $resultado = R::getAll($sql);
//        
//        $contenido = "";
...
//        }
//        $nombreZip = FCPATH . 'generadas/' . date('Y-m-d-H-i-s') . '-canjelocaltardioes.zip';
...
//    }
    ...
    public function previewMercado(){
        ...
        $sql = "SELECT  cierrecanjelocaltardio_id,
                        ...
                        FROM    canjelocaltardio
                WHERE   id in ({$ordenes_in})
                ORDER BY cierrecanjelocaltardio_id, plazo ";
        $resultado = R::getAll($sql);
...
//        $contenido = array();
        ...
        foreach ($resultado as $indice=>$fila){
            ...
            $plazo = R::findOne('plazocanjelocaltardio', 'cierrecanjelocaltardio_id = ? and id = ?', array($fila['cierrecanjelocaltardio_id'], $fila['plazo']));    
//            $plazo = R::getRow('SELECT * FROM plazocanjelocaltardio WHERE cierrecanjelocaltardio_id = '{$fila['cierrecanjelocaltardio_id']}' AND , $fila['plazo']));    
...
//                    break;
        ...
        foreach ($contenido as $data){
            ...
            $archivo = date('Y-m-d-H-i-s') . '-' . $data['colocacion'] . '-canjelocaltardio.txt';
            $this->zip->add_data($archivo, $data['datos']);
            //file_put_contents($archivo, $data['datos']);
            //array_push($uris, base_url() . 'generadas/' . basename($archivo));
        }
        $nombreZip = FCPATH . 'generadas/' . date('Y-m-d-H-i-s') . '-canjelocaltardio.zip';
        $this->zip->archive($nombreZip);
    ...
    public function previewArchivo(){
        ...
        $sql = "SELECT  
                        ...
                        cl.cuit,
                        cl.posicion
                FROM    canjelocaltardio cl
                LEFT JOIN plazocanjelocaltardio clpla
                ON cl.plazo = clpla.id
                AND cl.cierrecanjelocaltardio_id = clpla.cierrecanjelocaltardio_id
                WHERE cl.id in ({$ordenes_in})
                ORDER BY cl.cierrecanjelocaltardio_id, cl.plazo ";
        $resultados = R::getAll($sql);
        
        
//        [cierrecanjelocaltardio_id] => 1
//        [tipopersona] => FISICA
//        [numcomitente] => 24149
//        [cantidad] => 31000
//        [arancel] => 
...
//                    break;
            ...
            if ($minutos <= 10){
                ...
                $sql = "select  *
                        ...
                        where   id in (
                            ...
                            from   canjelocaltardio
                            where  cierrecanjelocaltardio_id = ?
                            and    estado_id = 1
                            order by usuario_id)";
    ...
    public function getPlazocanjelocaltardiobonoPorDescripcion(){
        $sql = "select id, plazocanjelocaltardio_id, plazodescripcion, descripcion, tipo, cierrecanjelocaltardio_id "
                . "from plazocanjelocaltardiobono "
                . "where descripcion = ? AND cierrecanjelocaltardio_id = ? AND plazocanjelocaltardio_id = ?";
    ...
    public function getPlazoPorNombre(){
        $sql = "select id, moneda, plazo, especie, colocacion, cierrecanjelocaltardio_id "
                . "from plazocanjelocaltardio "
                . "where plazo = ? AND cierrecanjelocaltardio_id = ?";
...
public function procesarExcel(){
        ...
        $cierre = R::load('cierrecanjelocaltardio', $this->cierre);
        $usuario = R::load('usuario', $usuarioParam['id']);
        $estadoorden = R::load('estadoorden', 1);        
...
//            $lasOrdenes = implode(",", $ordenes);
    ...
    public function grabarExcel(){
        ...
        $plazos = $this->Canjelocaltardio_model->getPlazos();
        ...
        if($aprobado){
            ...
            for ($row = 2; $row <= $highestRow; $row++){
                ...
                if (strlen(trim($numeroComitente)) > 0) {
                    ...
                    $orden = R::dispense('canjelocaltardio');
...
//                    }
                    ...
                    $this->Canjelocaltardio_model->plazoNombre = $plazoNombre;
                    $resultadoPlazo = $this->Canjelocaltardio_model->getPlazoPorNombre();
...
//                    }        
                    ...
                    $this->Canjelocaltardio_model->plazo = $plazo;
                    $this->Canjelocaltardio_model->bonoNombre = $bonoNombre;
                    $resultado = $this->Canjelocaltardio_model->getPlazocanjelocaltardiobonopordescripcion();
...
class Model_Canjelocaltardio extends RedBean_SimpleModel {
    private $prev;
    ...
    function after_update(){
        if (json_encode($this->prev) != json_encode($this->bean->export())){
            ...
            $auditoria->table = 'canjelocaltardio';
...
class Model_Cierrecanjelocaltardio extends RedBean_SimpleModel {
    private $prev;
    ...
    function after_update(){
        if (json_encode($this->prev) != json_encode($this->bean->export())){
            ...
            $auditoria = R::dispense('auditoria');
            $auditoria->usuario = $usuario;
            $auditoria->table = 'cierrecanjelocaltardio';
            $auditoria->tableId = $this->bean->id;
            $auditoria->anterior = json_encode($this->prev);
            $auditoria->actual = json_encode($this->bean->export());
            R::store($auditoria);
