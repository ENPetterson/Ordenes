...
//        $ordenAnterior = $orden;
        if ($orden->id == 0 ){
            ...
            if (isset($cierreActual['cerrado'])){
                ...
                $cierre = R::load('cierreminuta', $cierreActual['id']);
                $orden->cierreminuta = $cierre;
            }
        ...
        $orden->numRegistro = (int)$this->numRegistro;
        $orden->numComitente = (int)$this->numComitente;
        $orden->comitente = $this->comitente;
        $orden->fechaLiquidacion = $this->fechaLiquidacion;

        $orden->tipoOperacionBurs = $this->tipoOperacionBurs;
        $orden->tipoOperacionBursDesc = $this->tipoOperacionBursDesc;
        $orden->codEspecie = (int)$this->codEspecie;
        $orden->especieAbreviatura = $this->especieAbreviatura;
        $orden->codMoneda = (int)$this->codMoneda;
        $orden->monedaDescripcion = $this->monedaDescripcion;
        $orden->cantidad = (float)$this->cantidad;
//        $orden->especie = $this->especie;
//        $orden->moneda = $this->moneda;
        $orden->esComitenteCorregido = $this->esComitenteCorregido;
        $orden->numComitenteCorregido = (int)$this->numComitenteCorregido;
        $orden->comitenteCorregido = $this->comitenteCorregido;
        $orden->esCantidadCorregido = $this->esCantidadCorregido;
        $orden->cantidadCorregido = $this->cantidadCorregido;
        $orden->esArancelCorregido = $this->esArancelCorregido;
    ...
    public function enviarOrdenes(){
        ...
        foreach ($this->ordenes as $id) {
            ...
            $textoEmail = " <meta charset=\"utf-8\">
                            <p>Minutas </p>
                            <p>Estimado: </p>
                            <p>Se ha generado una nueva orden Minuta y enviado para su aprobación. {$id} {$usuario['nombre']} {$usuario['apellido']}</p>
                            <p>Saludos.</p>";
...
//            $respuesta = $this->Mailing_model->enviarMail($textoEmail, 'Allaria Ledesma', 'no-responder@allaria.com.ar', 'Ordenes Minuta', $mailTo, 'Ordenes Minuta', array(), array());
...
//            $fechaCierre = new DateTime($orden->cierreminuta->fechahora);
...
//            } else {
                ...
                $textoEmail = " <meta charset=\"utf-8\">
                                <p>Minutas </p>
                                <p>Estimado: </p>
                                <p>Se ha aprobado una nueva orden Minuta. {$id} {$usuario['nombre']} {$usuario['apellido']}</p>
                                <p>Saludos.</p>";
        ...
        foreach ($this->ordenes as $id) {
            ...
            if($resultadoAnular > 0){
                $textoEmail = " <meta charset=\"utf-8\">
                            <p>Minutas </p>
                            <p>Estimado: </p>
                            <p>Se ha anulado la Minuta; {$id} {$usuario['nombre']} {$usuario['apellido']}</p>
                            <p>Saludos.</p>";
...
//            $orden->controladores = 'Asd';
    ...
    public function getCierre(){
        $cierre = R::load('cierreminuta', $this->cierreminuta_id);
...
//        $cierre->colocacionA2J9 = $this->colocacionA2J9;
    ...
    public function getCierreActual(){
        $cierreActual = R::findOne('cierreminuta', 'fechahora > NOW() order by fechahora' );
    ...
    public function getCierres(){
        $cierres = R::getAll('select * from cierreminuta order by fechahora desc');
        return $cierres;
    }
    ...
    public function grilla(){
        $sql = "select l.id,
                ...
                l.fechaLiquidacion,
                l.tipoOperacionBurs,
                l.tipoOperacionBursDesc,

                l.codEspecie,
                l.especieAbreviatura,
         ...
         from minuta l
         join   cierreminuta c
         on     l.cierreminuta_id = c.id
         join   estadoorden eo
         on     l.estado_id = eo.id
         where  l.usuario_id = ?
         and    l.cierreminuta_id = ?
         order by l.fechaActualizacion desc";

        $resultado = R::getAll($sql, array($this->usuario_id, $this->cierreminuta_id));
    ...
    //      order by l.fechaActualizacion desc)";
    //
    //     $resultado = R::getAll($sql, array($this->cierreminuta_id)); //'1', 'Pendiente','2', 'Enviada','3', 'Procesada','4', 'Anulada','5', 'Retirada','6', 'Aprobada'
    //
    //     return $resultado;
    // }
    ...
    public function procesarGrilla(){
      $sql = "select l.id,
       ...
       join   estadoorden eo
       on     l.estado_id = eo.id
       where    l.cierreminuta_id = ?
       AND l.estado_id IN (2, 3, 4)
       order by l.fechaActualizacion desc";
    ...
    public function previewMercado(){
        ...
        $sql = "SELECT  cierreminuta_id,
                        ...
                        FROM    minuta
                WHERE   id in ({$ordenes_in})
                ORDER BY cierreminuta_id, moneda ";
        $resultado = R::getAll($sql);
        ...
        foreach ($resultado as $indice=>$fila){
            ...
            $cierre = R::load('cierreminuta', $fila['cierreminuta_id']);
...
//                    if ($fila['tramo'] == 'Competitiva') {
//                        $titulo = utf8_decode("minuta a 217 dias  Vto 13/09/2019  Tramo Competitivo");
//                        $precio = $fila['precio'];
//                    } else {
//                        $titulo = utf8_decode("minuta a 217 dias  Vto 13/09/2019 Tramo NO Competitivo");
//                        $precio = '';
//                    }
//                    $colocacion = $cierre['colocacionDolares'];
            // 2019/02/18
        ...
        foreach ($contenido as $data){
            ...
            $archivo = date('Y-m-d-H-i-s') . '-' . $data['colocacion'] . '-minuta.txt';
            $this->zip->add_data($archivo, $data['datos']);
        }
        $nombreZip = FCPATH . 'generadas/' . date('Y-m-d-H-i-s') . '-minuta.zip';
...
//                    $this->email->bcc(array('flagama@allariaycia.com','favio.fernandez@allariaycia.com','alejandro.oliveira@allariaycia.com','alfredo.daverede@allaria.com.ar'));
    ...
    public function enviarMailsCierreMinuta(){
        ...
        $fecha = new DateTime();
        $fechaHora = $fecha->format('H:i:s d-m-Y');
        // print ($fechaHora);
        // die();
        
        $sql = "SELECT * FROM ordenes.minuta
                WHERE cierreminuta_id = ANY (SELECT id FROM cierreminuta WHERE fechahora > DATE_FORMAT(CURDATE(), '%Y-%m-%d'))
                AND estado_id = 1;";
                $resultado = R::getAll($sql);
            foreach ($resultado as $minuta) {
            $textoEmail = "<html>
                             <p>$fechaHora</p>            
                             <p>Estimado/a </p>
                             <p>La siguiente minuta no fue enviada a Backoffice:</p>
                             <br>
                         <table>
                         <tr>
                             <td>Operador: </td>
                             <td>{$minuta['operador']}</td>
                         </tr>
                         <tr>
                             <td>Numero de registro: </td>
                             <td>{$minuta['numRegistro']}</td>
                         </tr>
                         </table>
                         </html>";
            ...
            // $mailTo = array('alejandro.oliveira@allaria.com.ar');
            $mailTo = array('andy.glustman@allaria.com.ar', 'alejandro.oliveira@allaria.com.ar');

            $respuesta = $this->Mailing_model->enviarMail($textoEmail, 'Allaria Ledesma', 'no-responder@allaria.com.ar', 'Minutas no enviadas', $mailTo, 'Ordenes Correccion', array(), array());
...
class Model_Minuta extends RedBean_SimpleModel {
    private $prev;
...
class Model_CierreMinuta extends RedBean_SimpleModel {
    private $prev;
