...
class Letes_model extends CI_Model{
    ...
    public function previewMercado(){
        ...
        $sql = "SELECT  cierreletes_id,
                        ...
                        FROM    letes
                WHERE   id in ({$ordenes_in})
                ORDER BY cierreletes_id, tramo, moneda, tipopersona, plazo ";
        $resultado = R::getAll($sql);
        $planillaAnterior = "";
        $plazoAnterior = 0;
        $filaPlanilla = 0;
    ...
    private function crearLibro(){
        $planillaTemplate =  FCPATH . 'template/letes2.xlsx';
    ...
    private function crearHoja($planilla, $plazo, $contador){
        ...
        $fecha = new DateTime();
        $this->workbook->getActiveSheet()->SetCellValue('E4', $fecha->format('d-M-Y'));
    ...
    private function escribirFila($planilla, $datos, $fila){
        ...
        */
         ...
         */
        $this->workbook->getActiveSheet()->setCellValue('C'.$filaExcel, $this->formatearCuit($datos['cuit']));
    ...
    private function nombreArchivo($nombrePlanilla){
        ...
        $resultado .= $plazo;
        $resultado .= ' DIAS ';
        if (substr($nombrePlanilla, 0, 1) <> 'P'){
