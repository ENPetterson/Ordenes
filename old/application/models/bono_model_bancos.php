...
class Bono_model extends CI_Model{
    ...
    */
    ...
    public function enviarOrdenes(){        
        ...
        foreach ($this->ordenes as $id) {
            $orden = R::load('bono', $id);
            $fechaCierre = new DateTime($orden->cierrebono->fechahora);
    ...
    public function saveCierre(){
        ...
        foreach ((array) $this->plazos as $plazoItem) {
            $plazo = R::load('plazobono', $plazoItem['id']);
            ...
            $plazo->cierrebono = $cierre;
            R::store($plazo);
        }
        
        foreach ((array) $this->plazosBorrar as $plazo_id){
            $plazo = R::load('plazobono', $plazo_id);
            R::trash($plazo);
        }
    ...
    public function delCierre(){
        $plazos = R::find('plazo', array($this->cierrebono_id));
        R::trashAll($plazos);
        $cierre = R::load('cierrebono', $this->cierrebono_id);
        R::trash($cierre);
    }
    ...
    public function getCierreActual(){
        ...
        if (is_null($cierreActual)){
            ...
            $this->cierrebono_id = $cierreActual->id;
    ...
    public function getPlazos(){
        if ($this->cierrebono_id > 0){
            $sql = "select plazo from plazobono where cierrebono_id = ? and moneda = ? order by plazo";
            $plazos = R::getCol($sql, array($this->cierrebono_id, $this->moneda));
        } else {
            $sql = "select plazo from plazobono where moneda = ? and cierrebono_id = (SELECT id FROM cierrebono where fechahora > NOW() order by fechahora limit 0,1)";
    ...
    public function getMonedas(){
        if ($this->cierrebono_id > 0){
            $sql = "select distinct moneda from plazobono where cierrebono_id = ? ";
            $monedas = R::getCol($sql, array($this->cierre_id));
        } else {
            $sql = "select distinct moneda from plazobono where cierrebono_id = (SELECT id FROM cierrebono where fechahora > NOW() order by fechahora limit 0,1)";
    ...
    public function previewSantander(){
        ...
        $sql = "SELECT  b.cierre_id,
                        ...
                        FROM    bono b
                        left outer join    plazobono p 
                        ON (b.cierrebono_id = p.cierrebono_id AND b.plazo = p.plazo AND b.moneda = p.moneda)
    ...
    public function previewMercado(){
        ...
        $sql = "SELECT  cierrebono_id,
                        ...
                        FROM    bono
                WHERE   id in ({$ordenes_in})
                ORDER BY cierrebono_id, moneda, plazo ";
        $resultado = R::getAll($sql);
        ...
        foreach ($resultado as $indice=>$fila){
            ...
            $contenido[$contenidoInd]['datos'] .= $colocacion . "\t" . $titulo . "\t" . $precio . "\t" . $fila['cantidad'] . "\t\t" . $fila['numcomitente'] . "\t" . $moneda . "\t" . $fila['cuit']  . "\t\t\t". $tipoInversor . "\t" . $tipoPersona . "\t\r\n";
        ...
        $nombreZip = FCPATH . 'generadas/' . date('Y-m-d-H-i-s') . '-bonos.zip';
